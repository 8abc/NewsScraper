...
// require our routes file pass out router object
...
app.get("/scrape", (req, res) => {
  ...
  axios.get("https://hypebae.com/").then(function(response) {
    ...
    $("a.post-link").each(function(i, element) {
      ...
      Article.find({ title: element.attribs.title }, function(err, result) {
        //if theres is (!) no result do the next code
        if (!result[0]._id) {
    ...
    // must be out of the .each scope so it's added all at once
